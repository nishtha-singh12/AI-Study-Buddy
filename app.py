...
with tab2:
    ...
    if user_input:
        ...
        # Convert existing tuple-based history into the dict format get_ai_response expects
        # (built BEFORE appending this turn, so the current question isn't duplicated)
        history_for_ai = [
        ...
        try:
            with st.spinner("Study Buddy is thinking..."):
                ai_reply = get_ai_response(user_input, profile_for_ai,chat_history=history_for_ai)
